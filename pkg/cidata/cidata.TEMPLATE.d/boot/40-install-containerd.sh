...
if [ "${LIMA_CIDATA_CONTAINERD_USER}" = 1 ]; then
	if [ ! -e "/home/${LIMA_CIDATA_HOMEDIR}/.config/containerd/config.toml" ]; then
		mkdir -p "/home/${LIMA_CIDATA_HOMEDIR}/.config/containerd"
		cat >"/home/${LIMA_CIDATA_HOMEDIR}/.config/containerd/config.toml" <<EOF
  version = 2
  [proxy_plugins]
...
EOF
		chown -R "${LIMA_CIDATA_USER}" "/home/${LIMA_CIDATA_HOMEDIR}/.config"
	fi
	selinux=
	if command -v selinuxenabled >/dev/null 2>&1 && selinuxenabled; then
		selinux=1
	fi
	if [ ! -e "/home/${LIMA_CIDATA_HOMEDIR}/.config/systemd/user/containerd.service" ]; then
