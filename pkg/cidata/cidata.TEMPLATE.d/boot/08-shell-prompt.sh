...
if [ "${LIMA_CIDATA_NAME}" = "default" ] && command -v patch >/dev/null 2>&1 && grep -q color_prompt "/home/${LIMA_CIDATA_HOMEDIR}/.bashrc"; then

	! grep -q "^# Lima PS1" "/home/${LIMA_CIDATA_HOMEDIR}/.bashrc" || exit 0

	# Change the default shell prompt from "green" to "lime" (#BFFF00)

	patch --forward -r - "/home/${LIMA_CIDATA_HOMEDIR}/.bashrc" <<'EOF'
@@ -37,7 +37,11 @@
